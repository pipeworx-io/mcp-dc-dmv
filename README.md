...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/dc_dmv_offices \
  -H 'Content-Type: application/json' \
  -d '{"name":"Georgetown"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/dc_dmv_offices`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
